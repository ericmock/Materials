...
$query = "select server_ready from gamedata where game_id = $game_id";
$debug_info .= $query;
//echo "query = $query \n";
$result = mysql_query($query);
$row = mysql_fetch_array($result);

if ($row['server_ready'] == 1) { // this is an existing game
	$query = "update gamedata set letters = \"$letters\" where game_id = $game_id";
	$debug_info .= $query;
	mysql_query($query);
} else { // this is a new game
	$query = "insert into gamedata set letters = \"$letters\", polyhedron = $poly_id, game_id = $game_id, time_stamp = now(), server_ready = 1";
	$debug_info .= $query;
	mysql_query($query);
