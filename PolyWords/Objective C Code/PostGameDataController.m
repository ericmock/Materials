#import "PostGameDataController.h"
#import "AppController.h"
#import "WebViewController.h"
#import "EnDe.h"

@implementation PostGameDataController
...
NSLog(@"into startSend:(NSArray *)informationArray  of %@",[self class]);
	...
	url = [NSURL URLWithString:@"http://www.smallfeats.com/polywords/gamedata.php"];
    success = (url != nil);

	if (success) {
		request = [NSMutableURLRequest requestWithURL:url];
		NSString *dataString = [NSString stringWithFormat:@"Letters=%@&PolyhedronID=%@&Mode=%@",[informationArray objectAtIndex:0],[[informationArray objectAtIndex:1] objectForKey:@"polyID"],[informationArray objectAtIndex:2]];
		AppController *appController = (AppController *)[[UIApplication sharedApplication] delegate];
		NSUInteger hash = [dataString hash];
		appController.game_id = hash;
		NSString *postString = [NSString stringWithFormat:@"%@&GameID=%u",dataString,hash];
