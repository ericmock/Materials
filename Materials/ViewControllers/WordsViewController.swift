...
//		let cell:UITableViewCell = tableViewCellWithReuseIdentifier(identifier)//[self tableviewCellWithReuseIdentifier:identifier]
	...
	func configure(withCell cell:UITableViewCell, forIndexPath path:IndexPath) {
		let wordsArray = [["Best words", "Longest words", "Average word scores", "Average word lengths"],
...
//		let imagesArray = ["Tetrahedron.png", "Cube.png", "Octahedron.png", "Dodecahedron", "Icosahedron", "Cuboctahedron"]
	...
	func tableView(_ tableView: UITableView, willSelectRowAt index:IndexPath) -> IndexPath? {
	...
	/*
	 ...
	 //	WordsScrollerController *wordsScrollerController = [[WordsScrollerController alloc] initWithWords:newIndexPath.row];
	 
	 //	[[self navigationController] pushViewController:wordsScrollerController animated:YES];
	 //	[wordsScrollerController release];
	 return newIndexPath;
	 }
	 */
}
