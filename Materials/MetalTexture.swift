...
class MetalTexture: NSObject {
  ...
  init(resourceName: String,ext: String, mipmaped:Bool){
    ...
    url = Bundle.main.url(forResource: resourceName, withExtension: "png")
    width    = 0
    height   = 0
    depth    = 1
  ...
  func loadTexture(device: MTLDevice, commandQ: MTLCommandQueue, flip: Bool){
    ...
    // need to make url from path
    let image = CIImage(contentsOf: url)!//(UIImage(contentsOfFile: path)?.cgImage)!
    ...
    context.draw(image.cgImage!, in: bounds, byTiling: false)
  ...
  class func textureCopy(source:MTLTexture,device: MTLDevice, mipmaped: Bool) -> MTLTexture {
    ...
    let copyTexture:MTLTexture = device.makeTexture(descriptor: texDescriptor)!
    ...
    let bytePerRow:Int = Int(source.width) * 4
    copyTexture.replace(region: region, mipmapLevel: 0, withBytes: pixelsData, bytesPerRow: bytePerRow)
    return copyTexture
  }
  ...
  func image(mipLevel: Int) -> CIImage{
    ...
    let image = CIImage(cgImage: imgRef!)
    return image
  }
